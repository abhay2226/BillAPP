...
const getCode = async <T extends ObjectLiteral>(
  ...
  if (!row) {
    throw new Error(
      `${label} with code '${code}' not found or inactive.`
    );
  }
...
const generateInvoiceNumber = async (
  ...
  const seq = await manager
    ...
    .andWhere(
      "bill.created_at >= :startOfDay",
      { startOfDay }
    )
    .getCount();

  return `INV-${store_id}-${datePart}-${String(
    seq + 1
  ).padStart(4, "0")}`;
};
...
// ======================================================
// VALIDATE + PRICE CART ITEMS
// ======================================================

async function validateAndPriceCartItems(
  manager: EntityManager,
  storeId: number,
  items: BillItemInput[]
) {
  ...
  for (const item of sortedItems) {
    ...
    if (inventory.store_id !== storeId) {
      throw new Error(
        `Inventory ${item.inventory_id} does not belong to store ${storeId}.`
      );
    }
    ...
    const unit_price =
      Number(inventory.selling_price);

    const line_total =
      round2(unit_price * item.qty);

    subtotal =
      round2(subtotal + line_total);
...
async function createBillItemRows(
  ...
  for (const line of lineItems) {
    ...
    const billItem = manager.create(
      BillItem,
      {
        ...
        product_name_snapshot:
          line.inventory.product?.product_name ?? "",
        qty: line.qty,
    ...
    await createAuditRecordService(
      ...
      {
        tableName:
          "transactions_bill_item",
        recordId:
          savedBillItem.bill_item_id,
...
export async function getBillItemsForBill(
  ...
  return items.map((item) => ({
    ...
    productId:
      item.inventory?.product?.product_id ??
      null,
  ...
  return AppDataSource.transaction(
    async (manager) => {
      ...
      const {
        ...
        await validateAndPriceCartItems(
          manager,
          input.store_id,
          input.items
        );
      ...
      if (input.discount_id) {
        ...
        if (
          discount.store.store_id !==
          input.store_id
        ) {
          throw new Error(
            "This discount does not belong to the selling store."
          );
        }

        bill_discount_total =
          await resolveDiscountAmount(
      ...
      if (grand_total < 0) {
        throw new Error(
          "Grand total cannot be negative — check discount configuration."
        );
      }
...
export async function getBillHistory(
  ...
  if (filters.invoiceNumber) {
    ...
    qb.andWhere(
      "bill.invoice_number LIKE :invoiceNumber",
      {
        invoiceNumber:
  ...
  if (filters.date) {
    ...
    qb.andWhere(
      "bill.created_at >= :start AND bill.created_at < :end",
      {
        start,
        end
      }
